...
class HoraExtra
{
    ...
    public function calcularHoraTrabalhada($pontos)
    {
        if (isset($pontos)) {
            dd($pontos->all());
        ...
        // ----------------------------------
        $dt = new Carbon();
        $k1 = explode(':', $pontos->entrada);
        $k2 = explode(':', $pontos->entrada_almoco);
        $k3 = explode(':', $pontos->saida_almoco);
        $k4 = explode(':', $pontos->saida);
        ...
        // ----------------------------------
        $entrada = new Carbon($pontos->entrada);
        
        // ----------------------------------
        // adicionando as horas de entrada do almoco
        // ----------------------------------
        $entrada_almoco = new Carbon($pontos->entrada_almoco);
        
        // ----------------------------------
        // construindo a hora de retoro do almoco
        // ----------------------------------
        $saida_almoco =  new Carbon($pontos->saida_almoco);
        
        // ----------------------------------
        // construindo a hora de saida
        // ----------------------------------
        $saida =  new Carbon($pontos->saida);
    ...
    public function calcularHoraExtra($request = null)
    {
        ...
        // ------------------------------------------------------------------------
        // Recebendo a diferença de horas entre carga horaria e horas trabalhadas
        // ------------------------------------------------------------------------
        ...
        // ------------------------------------------------------------------------
        return $verificacao = [explode(' ', $result_extras), $horas_trabalhadas];
